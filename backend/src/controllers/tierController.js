...
// Support levels: "Support Level" on screen, project_tiers in the database, `tier` in the
// code. Routes are unchanged, so these still answer /api/projects/:id/tiers.
...
const deleteTier = asyncHandler(async (req, res) => {
    // `hidden` tells the UI which happened: a level nobody chose is really gone, while
    // one somebody chose is hidden so their history still points at a row that exists.
